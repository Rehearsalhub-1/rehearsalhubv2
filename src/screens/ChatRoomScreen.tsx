    ...
    showToast('Message unpinned');
        ...
        <Modal visible={showGifPicker} transparent animationType="slide" onRequestClose={() => setShowGifPicker(false)}>
          <View style={{ flex: 1, backgroundColor: theme.colors.background, marginTop: 60, borderTopLeftRadius: 20, borderTopRightRadius: 20 }}>
            <View style={{ padding: 16, flexDirection: 'row', alignItems: 'center', borderBottomWidth: 1, borderBottomColor: theme.colors.border }}>
              <TouchableOpacity onPress={() => setShowGifPicker(false)} style={{ marginRight: 12 }}><Ionicons name="close" size={24} color={theme.colors.textPrimary} /></TouchableOpacity>
              <TextInput style={{ flex: 1, backgroundColor: theme.colors.cardBackground, color: theme.colors.textPrimary, padding: 8, borderRadius: 8 }} placeholder="Search Tenor GIFs..." placeholderTextColor={theme.colors.textMuted} value={gifQuery} onChangeText={(text) => { setGifQuery(text); searchGifs(text); }} autoFocus />
            </View>
            {gifLoading ? (
              <ActivityIndicator style={{ marginTop: 20 }} color={APP_THEME.primaryAccent} />
            ) : (
              <FlatList data={gifResults} numColumns={2} keyExtractor={item => item.id} renderItem={({item}) => (
                <TouchableOpacity onPress={() => sendGif(item.url)} style={{ flex: 1, padding: 2 }}>
                  <Image source={{ uri: item.preview }} style={{ width: '100%', height: 120, borderRadius: 8 }} contentFit="cover" />
                </TouchableOpacity>
              )} />
            )}
          </View>
        </Modal>
        <Modal visible={showContactShareModal} transparent animationType="slide" onRequestClose={() => setShowContactShareModal(false)}>
          <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'flex-end' }}>
            <View style={{ backgroundColor: theme.colors.cardBackground, borderTopLeftRadius: 20, borderTopRightRadius: 20, padding: 20, minHeight: 300 }}>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                <Text style={{ color: theme.colors.textPrimary, fontSize: 18, fontWeight: 'bold' }}>Share Contact</Text>
                <TouchableOpacity onPress={() => setShowContactShareModal(false)}><Ionicons name="close" size={24} color={theme.colors.textPrimary} /></TouchableOpacity>
              </View>
              <Text style={{ color: theme.colors.textMuted, marginBottom: 20, textAlign: 'center' }}>System contacts integration would appear here. For now, you can share your own contact info.</Text>
              <TouchableOpacity onPress={() => handleShareContact({ uid: currentUser?.uid || '', name: myName || 'Me', avatar: '' })} style={{ backgroundColor: APP_THEME.primaryAccent, padding: 14, borderRadius: 8, alignItems: 'center' }}>
                <Text style={{ color: theme.colors.textPrimary, fontWeight: 'bold' }}>Share My Contact</Text>
              </TouchableOpacity>
            </View>
          </View>
        </Modal>
        <Modal visible={showDisappearingModal} transparent animationType="slide" onRequestClose={() => setShowDisappearingModal(false)}>
          <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'flex-end' }}>
            <View style={{ backgroundColor: theme.colors.cardBackground, borderTopLeftRadius: 20, borderTopRightRadius: 20, padding: 20 }}>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                <Text style={{ color: theme.colors.textPrimary, fontSize: 18, fontWeight: 'bold' }}>Disappearing Messages</Text>
                <TouchableOpacity onPress={() => setShowDisappearingModal(false)}><Ionicons name="close" size={24} color={theme.colors.textPrimary} /></TouchableOpacity>
              </View>
              {[
                ...
                <TouchableOpacity key={opt.label} style={{ flexDirection: 'row', alignItems: 'center', paddingVertical: 14, borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: theme.colors.border }} onPress={() => handleSetDisappearing(opt.value)}>
                  <View style={{ width: 20, height: 20, borderRadius: 10, borderWidth: 2, borderColor: disappearingTimer === opt.value ? APP_THEME.primaryAccent : theme.colors.textMuted, alignItems: 'center', justifyContent: 'center', marginRight: 12 }}>
                    {disappearingTimer === opt.value && <View style={{ width: 10, height: 10, borderRadius: 5, backgroundColor: APP_THEME.primaryAccent }} />}
                  </View>
                  <Text style={{ color: theme.colors.textPrimary, fontSize: 16 }}>{opt.label}</Text>
                </TouchableOpacity>
              ))}
            </View>
          </View>
        </Modal>
        <Modal visible={showWallpaperModal} transparent animationType="slide" onRequestClose={() => setShowWallpaperModal(false)}>
          <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'flex-end' }}>
            <View style={{ backgroundColor: theme.colors.cardBackground, borderTopLeftRadius: 20, borderTopRightRadius: 20, padding: 20 }}>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                <Text style={{ color: theme.colors.textPrimary, fontSize: 18, fontWeight: 'bold' }}>Chat Wallpaper</Text>
                <TouchableOpacity onPress={() => setShowWallpaperModal(false)}><Ionicons name="close" size={24} color={theme.colors.textPrimary} /></TouchableOpacity>
              </View>

              <Text style={{ color: theme.colors.textSecondary, marginBottom: 8 }}>Dimming (Opacity)</Text>
              <Slider
                ...
                onValueChange={(val) => setChatWallpaperOpacity(val)}
                onSlidingComplete={async (val) => {
                  await AsyncStorage.setItem(`wallpaper_opacity_${room?.id}`, val.toString());
                }}
                minimumTrackTintColor={APP_THEME.primaryAccent}
                maximumTrackTintColor={theme.colors.border}
                thumbTintColor={APP_THEME.primaryAccent}
              />

              <TouchableOpacity onPress={handlePickWallpaper} style={{ backgroundColor: APP_THEME.primaryAccent, padding: 14, borderRadius: 8, alignItems: 'center', marginBottom: 12 }}>
                <Text style={{ color: theme.colors.textPrimary, fontWeight: 'bold' }}>Choose from Gallery</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={handleRemoveWallpaper} style={{ padding: 14, borderRadius: 8, alignItems: 'center', borderWidth: 1, borderColor: '#ef4444' }}>
                <Text style={{ color: '#ef4444', fontWeight: 'bold' }}>Remove Wallpaper</Text>
              </TouchableOpacity>
            </View>
          </View>
        </Modal>
        <Modal visible={showJoinLinkModal} transparent animationType="slide" onRequestClose={() => setShowJoinLinkModal(false)}>
          <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'center', alignItems: 'center' }}>
            <View style={{ backgroundColor: theme.colors.cardBackground, borderRadius: 20, padding: 24, width: '85%', alignItems: 'center' }}>
              <Text style={{ color: theme.colors.textPrimary, fontSize: 18, fontWeight: 'bold', marginBottom: 16 }}>Group Invite Link</Text>
              <View style={{ backgroundColor: theme.colors.background, padding: 16, borderRadius: 8, width: '100%', marginBottom: 16 }}>
                <Text style={{ color: APP_THEME.primaryAccent, textAlign: 'center' }}>{joinLink}</Text>
              </View>
              <View style={{ flexDirection: 'row', gap: 12, width: '100%' }}>
                <TouchableOpacity style={{ flex: 1, padding: 12, borderRadius: 8, backgroundColor: theme.colors.background, alignItems: 'center' }} onPress={() => setShowJoinLinkModal(false)}>
                  <Text style={{ color: theme.colors.textPrimary }}>Close</Text>
                </TouchableOpacity>
                <TouchableOpacity style={{ flex: 1, padding: 12, borderRadius: 8, backgroundColor: APP_THEME.primaryAccent, alignItems: 'center' }} onPress={async () => {
                  try {
                    const { Clipboard } = require('react-native');
                  ...
                  <Text style={{ color: theme.colors.textPrimary, fontWeight: 'bold' }}>Copy Link</Text>
                </TouchableOpacity>
              </View>
            </View>
          </View>
